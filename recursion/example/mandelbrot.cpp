...
//****************************************
GLfloat* calculateColor2 (GLfloat u, GLfloat v) {
    GLfloat re = u;
    GLfloat im = v;
    GLfloat tempRe=0.0;
    for (int i=0; i < paletteSize; i++) {
...
void reshape (int w, int h){ // function called when window size is changed
    // calculate new value of step along X axis
    stepX = (maxX-minX)/(GLfloat)w;
    // calculate new value of step along Y axis
    stepY = (maxY-minY)/(GLfloat)h;
    // set new dimension of viewable screen
