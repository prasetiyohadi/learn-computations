...
#include <iostream>
#include <vector>
// f1(x1,x2) = (1-0.25/pi)*(e^(2*x1)-e) + e*x2/pi - 2*e*x1 = 0
// df1(x1,x2)/dx1 = (1-0.25/pi)*(2*e^(2*x1)) - 2*e
// df1(x1,x2)/dx2 = e/pi
// f2(x1,x2) = 0.5*sin(x1*x2) - 0.25*x2/pi - 0.5*x1 = 0
// df2(x1,x2)/dx1 = 0.5*x2*cos(x1*x2) - 0.5
// df2(x1,x2)/dx2 = 0.5*x1*cos(x1*x2) - 0.25/pi
...
int main () {
    ...
    while (loop1) {
        loop2 = true;
        a = {{(1-0.25/M_PI)*2*std::exp(2*xt[0])-2*std::exp(1), 
            std::exp(1)/M_PI}, 
            {0.5*xt[1]*std::cos(xt[0]*xt[1])-0.5, 
